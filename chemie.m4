...
electron pair to an electrophile to form a chemical
bond in relation to a reaction. All molecules or
ions with a free pair of electrons or at
...
Most electrophiles are positively charged, have an atom
that carries a partial positive charge, or have an
atom that does not have an octet of electrons.

The electrons that determine how an atom reacts
chemically are those that travel farthest from the
nucleus, that is those with the highest energy.
...
The formal **oxidation state** is the hypothetical charge
...
\chemright]^3+^
+
\chemabove{\ce{H3O}}{\scriptstyle\oplus}
\schemestop

\pKa = 3.8

Salts of small highly charged metal cations
are acidic.
...
No electrons in an atom can have all
...
Hund's rules:
- The state with the largest values of S is the
most stable.
...
the more abundant chromite, chrome yellow was, together with cadmium yellow, one
of the most used yellow pigments. The pigment does not photodegrade, but
it tends to darken due to the formation of chromium(III) oxide. It
has a strong color, and was used for school buses in the
US and for Postal Service (for example Deutsche Post) in Europe. The
use of chrome yellow declined due to environmental and safety concerns and
was replaced by organic pigments or alternatives free from lead and chromium.
Other pigments based on chromium are, for example, the bright red pigment
chrome red, which is a basic lead chromate (PbCrO4·Pb(OH)2). A very important
chromate pigment, which was used widely in metal primer formulations, was zinc
chromate, now replaced by zinc phosphate. A wash primer was formulated to
replace the dangerous practice of pretreating aluminium aircraft bodies with a phosphoric
acid solution. This used zinc tetroxychromate dispersed in a solution of polyvinyl
butyral. An 8% solution of phosphoric acid in solvent was added just
before application. It was found that an easily oxidized alcohol was an
...
Seven nonmetals exist as diatomic molecules in their standard states: hydrogen (H2);
...
Proton transfers are fast, exept for
Carbon (keine guten H-Bonds).
...
carboxyl group (C(O)OH).[1] The general formula of a carboxylic acid is
...
Strong acid with strong base: H^+^ + OH^-^ -> H~2~O.
...
pK~ion~ $= -\log$ K~ion~ $= -\log$( \ce{[H3O+][OH-]} ).
...
\schemestart
\chemname{\ce{ZnO}}{borderline\\acid\\hard base}
\+
\chemname{\ce{2LiC4H9}}{hard acid\\soft base}
\arrow
\chemname{\ce{Zn(C4H9)_2}}{borderline\\acid\\soft base}
\+
...
Acid salt is a term for a class of salts formed by
the partial neutralization of diprotic or polyprotic acids. Because the parent acid
is only partially neutralized, one or more replaceable hydrogen atoms remain. Typical
acid salts have one or more alkali (alkaline) metal ions as well
as one or more hydrogen atoms. Well known examples are sodium bicarbonate
(NaHCO3), sodium hydrosulfide (NaHS), sodium bisulfate (NaHSO4), monosodium phosphate (NaH2PO4), and disodium
phosphate (Na2HPO4). Often acid salts are used as buffers.[1]
...
of the molecule.
Only irreducible representaions with
$x$ or $y$ or $z$ symmetry do this.

N~2~O~4~: B~1u~, 2B~2u~, 2B~3u~ (5 modes)

\minisec{Raman active}
The vibration must change the polatizability
of the molecule.
Only irreducible representaions that transform
like the binary products of $x$, $y$ and $z$
(i.e. $xy$, $xz$, $yz$, $x^2$, $y^2$, $z^2$ or
...
Promotors are additives which maximize the
surface area of the catalyst and enhance its resistance to poisoning and thermal deac-
tivation. 
...
Singly promoted catalysts contain
only \ce{Al2O3} , whereas doubly promoted catalysts contain \ce{K2O} as well. In addition,
there are low levels of a number of other additives, some of which originated as im-
purities in the original ore and were found to enhance the activity of the catalyst.
Before the magnetite is reduced to (mainly) iron, it is
fused with the promotors at 1600 8C to give a low area, non porous Al-magnetite
with \ce{K2O} located at the grain boundaries of the magnetite crystals. The fused catalyst
is then reduced with \ce{H2} or a mixture of \ce{H2} and \ce{CO} (synthesis gas) to give a porous
a-Fe matrix with a surface area of 10±25 m 2 g ±1 . The structural promotor (\ce{Al2O3}) is
distributed over the surface of the Fe particles and stabilises them against sintering
...
Als Sublimation, seltener auch Sublimierung (von
lat. sublimis = ‚hoch in der Luft befindlich‘,
‚erhaben‘), bezeichnet man in der Thermodynamik den
